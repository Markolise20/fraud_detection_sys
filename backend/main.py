...
# To get started, deploy with `firebase deploy`
...
try:
    cred_data = json.loads(os.environ.get('FIREBASE_CREDENTIALS', '{}'))
    if not cred_data or 'type' not in cred_data:
        raise ValueError("FIREBASE_CREDENTIALS environment variable not set or invalid")
...
def validate(req: https_fn.Request) -> https_fn.Response:
    # Convert Flask response to https_fn.Response
    from flask import Response as FlaskResponse
    flask_response = app(req.environ, lambda status, headers, body: (status, headers, [body]))
    return https_fn.Response(
        response=flask_response[2][0],
        status=flask_response[0],
        headers=dict(flask_response[1])
    )